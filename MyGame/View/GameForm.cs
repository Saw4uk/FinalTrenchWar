    ...
    public partial class Form1 : Form
    {
        ...
        private void Update(object sender, EventArgs e)
        {
            ...
            foreach (var unit in GameModel.AllUnits)
            {
                switch (unit.IsEnemy)
                {
                    case true when unit.PosX == 86:
                        Lost();
                        break;
                    case false when unit.PosX == 1000:
                        Win();
                        break;
                }
                if (unit.IsAlive == 0)
            ...
            if (GameModel.EnemyKilled % 25 == 0 && GameModel.EnemyKilled != 0)
            {
                GameModel.EnemyKilled += 1;
                GameModel.PlayerSecretDocuments += 1;
            }

            foreach (var pair in GunnersButtons)
            {
                if (pair.Key.IsAlive == 0)
                {
        ...
        public void Win()
        {
            Interface.MainPlayer.Stop();
            Player.openPlayer(ViewGraphics.WinVideo);
            Close();
        }
        ...
        {
            Interface.MainPlayer.Stop();
            Player.openPlayer(ViewGraphics.LostVideo);
            Close();
        }
        public void CheckInterface()
        {
            //Тернарники применить не дает, говорит нельзя к изменениям свойств.
